...
public class RagAdminService {
    ...
    final MilvusClientV2 milvusClient;
    final Environment env;
    final RagVectorSearchService ragVectorSearchService;
    final RagVectorIndexService ragVectorIndexService;

    /**
     * 管理端检查与分页查询向量数据；支持语义检索分页。
     */
    public R inspect(Integer page, Integer size, String message) {
        ...
        if (message != null && !message.trim().isEmpty()) {
            try {
                java.util.List<RagVectorSearchService.SearchHit> all =
        ...
        try {
            ...
            List<RagVectorSearchService.SearchHit> items = new ArrayList<>();
            if (results != null) {
                for (QueryResp.QueryResult r : results) {
                    Map<String, Object> entity = r.getEntity();
                    RagVectorSearchService.SearchHit h =
                            new RagVectorSearchService.SearchHit();
                    if (entity != null) {
     ...
     * 触发向量索引重建（全量或限制前N篇）。
     */
    public R reindex(Integer limit) {
        try {
