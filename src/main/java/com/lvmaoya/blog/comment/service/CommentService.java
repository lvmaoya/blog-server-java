...
import com.lvmaoya.blog.comment.mapper.CommentMapper;
...
public class CommentService extends ServiceImpl<CommentMapper, Comment> {
    ...
    @Transactional
    public R addOrUpdateComment(CommentForm commentForm, HttpServletRequest request) {
        Blog blog = blogMapper.selectById(commentForm.getArticleId());
        if (blog == null) {
            throw new BusinessException(400, "没有这篇文章");
        }
        if(Objects.isNull(commentForm.getUsername())){
            throw new BusinessException(400, "用户名必填");
        }
        // 获取用户的ip，到用户库里查询，如果没有该用户且该用户的name没填需要抛出异常
        String clientIp = ipUtils.getClientIp(request);
        CommentUser commentUser = commentUserMapper.selectById(clientIp + ":" + commentForm.getUsername());
        if(Objects.isNull(commentUser) && Objects.isNull(commentForm.getUsername())){
            throw new BusinessException(400, "用户名必填");
        }
        CommentUser commentUserInsert = BeanCopyUtil.copyBean(commentForm, CommentUser.class);
        commentUserInsert.setId(clientIp + ":" + commentForm.getUsername());

        commentUserMapper.insertOrUpdate(commentUserInsert);

        // 构建邮件内容
        String subject = commentForm.getType() == 0 ?
                "LvmaoyaBlog - 您收到了新的文章评论" :
                "LvmaoyaBlog - 您收到了新的回复";

        String content;
        String toEmail;
        if (commentForm.getType() == 0) {
            // 文章评论邮件
            content = String.format(
                    ...
                    "https://lvmaoya.cn/detail/" + commentForm.getArticleId()
            );
            toEmail = "1504734652@qq.com";
        } else {
            // 评论回复的那个邮件
            Comment toComment = commentMapper.selectById(commentForm.getToCommentId());
        ...
        Comment comment = BeanCopyUtil.copyBean(commentForm, Comment.class);
        comment.setUserId(clientIp  + ":" + commentForm.getUsername());
    ...
    public R selectList(CommentQuery params) {
    ...
    public R updateCommentStatus(CommentStatusUpdateForm dto) {
